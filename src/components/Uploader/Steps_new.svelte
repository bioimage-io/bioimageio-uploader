<!--
  ...
  From: https://raw.githubusercontent.com/shaozi/svelte-steps/master/src/lib/Steps.svelte
  ...
  - `clickables`: When set to `false`, Clicking icons and labels will not change step. You have to change `current` to change step. Default `true`
...
<script lang="ts">
  ...
  let onClick = (i: number) => {
    let clickable = true;
    if(clickables) clickable = clickables[i];
    if (clickable) {
      let last = current
      current = i
      // $progress = i
...
>
  ...
  >
    {#each steps as step, i}
      <!-- step container -->
      {@const clickable = (clickables === null) ? true : clickables[i] } 
      <div
